...
// Dynamically create the enum from the available languages
...
}).refine(data => data.favorites !== undefined || data.languageName !== undefined || data.localeId !== undefined, {
    message: "At least one property to update (favorites, languageName, or localeId) must be provided."
});
...
IMPORTANT: When updating favorites, this operation replaces the entire existing list with the new one you provide. To add or remove a single favorite, you should first use the 'getUserProfile' tool to get the current list, modify it, and then provide the complete, updated list to this tool.`,
    ...
    execute: async (params: UpdateUserProfileInput, context: any) => {
        ...
        const { userId, favorites, languageName, localeId } = params;
        ...
        try {
            ...
            const userProfilePayload = getProfileResponse.data;
            ...
            if (favorites !== undefined) {
                ...
                if (!userProfilePayload.Preferences) {
                    userProfilePayload.Preferences = { "$type": "UserPreferences" };
                }
            ...
            if (languageName !== undefined && userProfilePayload.User) {
                ...
                if(languageId !== undefined) {
                    userProfilePayload.User.LanguageId = languageId;
                }
            }
