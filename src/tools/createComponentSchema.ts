...
Both of these properties are arrays where:
  - Each item is a Field Definition object that specifies the field's type, properties, and 'Name' (its machine name).
  - The order of the items in the array dictates the order of the fields in the resulting Schema.
...
Each Field Definition object MUST include a 'type' property to identify its type from the list below. Other common properties include:
  - Name: The machine name of the field.
...
Supported Field Types ('type' values):
  ...
  - NumberFieldDefinition: A field for numeric values. The 'MinValue', 'MaxValue', 'TotalDigits', and 'FractionDigits' properties can be used to restrict the range of values.
  - DateFieldDefinition: A field for date/time values. The date range can be restricted using properties like 'MinValue' and 'MaxValue'.
