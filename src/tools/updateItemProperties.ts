...
const updateItemPropertiesInputProperties = {
    ...
    fields: z.record(fieldDefinitionSchema).optional().describe("For Schema updates only. A dictionary of field definitions for the Schema's content. Replaces the existing fields. For modifying specific properties of existing fields (such as their description or making a field optional), the more efficient 'updateSchemaFieldProperties' tool is recommended. Use 'updateItemProperties' for wholesale structural changes like adding, removing, or reordering fields, which requires replacing the entire field collection."),
    metadataFields: z.record(fieldDefinitionSchema).optional().describe("For Schema updates only. A dictionary of field definitions for the Schema's metadata. Replaces the existing metadata fields. For modifying specific properties of existing metadata fields (such as their description or making a field optional), the more efficient 'updateSchemaFieldProperties' tool is recommended. Use 'updateItemProperties' for wholesale structural changes like adding, removing, or reordering fields, which requires replacing the entire field collection."),
