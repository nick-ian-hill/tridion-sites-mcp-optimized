...
To find items that are shared and can be localized, use the 'search' tool with the 'BlueprintStatus' parameter set to 'Shared'.`,
    input: {
        itemIds: z.array(z.string().regex(/^(tcm:\d+-\d+(-\d+)?|ecl:[^:\s]+)$/)).describe("An array of unique IDs (TCM URIs) for the shared items to be localized. Use the 'search' tool to find items with a 'BlueprintStatus' of 'Shared'."),
    },
